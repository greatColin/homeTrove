...
| M1-5 | `vlm.qwen3vl` 中文描述 | [x] 2026-08-10 | M1-3（视频） | **双后端**：真实 VLM 走 OpenAI 兼容端点（`endpoint_url`），缺失时回退确定性 mock（与 `asr.faster_whisper` 一致）；图片→1 条 `t=None` caption；视频按 `basic.scene_detect` 场景关键帧逐场景生成带时间戳 caption；输出 `{captions:[{t,caption}]}` 落 `plugin_results`；**caption scope 追加**：`embedding.bge_m3` 文本向量插件（依赖 `vlm.qwen3vl`），将描述编码为 `scope=caption` 向量写入 `embeddings`+sqlite-vec，`scope:caption` 前缀与关键词召回可用，视频命中 `can_seek=true` |
| M1-6 | `embedding.jina_clip` + `embedding.bge_m3` | [x] 2026-08-05（基础设施 + image/scene scope） | M1-5（caption scope ✅ 2026-08-10） | sqlite-vec 接入；`embeddings` 表（scope=image/scene/**caption**）；**mock 向量阶段**（确定性哈希，颜色/文本特征），`_encode_image/_encode_frame`/`_encode_text` 原地替换为真实模型即得真实语义 |
| M1-7 | `/search` 语义搜索（双路召回 + RRF 融合） | [x] 2026-08-05（mock 向量阶段） | M1-6 | **caption 关键词+向量召回随 M1-5 并入**：`scope:caption` 前缀限定向量召回；`vlm.qwen3vl` 加入关键词路径 LIKE 匹配（caption 文本）；视频 caption 命中带 `t_start/t_end` + `can_seek`；其他同前 |
...
## v1 视频栅格静音预览（桌面 hover）

> 2026-08-07 完成。

**目标**：时间轴 justified 网格中，桌面端把鼠标悬停到视频缩略图上即静音原地播放预览，移开即停止，不点开也能快速了解视频内容。

**规格文档**：`.monkeycode/specs/v1-muted-preview/requirements.md` + `design.md`
...
- 最后一勾：**M1 插件扩展**。本线已完成 M1-1~M1-11，**M1-5 `vlm.qwen3vl` + caption scope ✅**（双后端 auto|mock|qwen3vl；图片/视频场景描述；`embedding.bge_m3` caption 文本向量 + `scope:caption` 搜索 + 关键词召回；134 测试全绿）。**M1-7 已实现双路召回 + RRF 融合 + 视频跳秒**（caption 文本与向量随 M1-5 并入）。剩余主线：真实模型替换（jina-clip-v2、faster-whisper、InsightFace 已有）+ v1.1/v2 功能。
