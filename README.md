...
- 已覆盖：时间轴基础版、文件夹视图、灯箱、标签页（自动标签待 M1-5）、人物核心（命名/合并/过滤）、索引扫描、索引进度可视化、失败重试、健康检查、单用户、相册、语义搜索（mock 向量）、场景切分、插件管理、鉴权骨架、ASR 字幕（默认 mock，装 `faster-whisper` 自动切真模型）、**回收站**、**批量操作**、**收藏**、**共享相册**、**justified 网格 + 虚拟滚动**、**智能相册**、**地点地图**、**高级筛选**
- 待补齐（M1 完成后）：以图搜图、关键帧、静音预览、跳秒播放
...
**下一项：高级筛选 ✅；剩余 v1 必备：以图搜图 / 关键帧 / 静音预览 / 跳秒播放。推荐下一项为「以图搜图」——后端已有 VLM 描述/标签基础，v1 简化版可从某张照片出发召回语义相近资产（CLIP/embedding 余弦相似），前端在资源详情页加「找相似」入口并复用 JustifiedGrid 展示结果。**
