...
### v1 必备（M0/M1 已覆盖的项）
- 已覆盖：时间轴基础版、文件夹视图、灯箱、标签页（自动标签待 M1-5）、人物核心（命名/合并/过滤）、索引扫描、索引进度可视化、失败重试、健康检查、单用户、相册、语义搜索（mock 向量）、场景切分、插件管理、鉴权骨架、ASR 字幕（默认 mock，装 `faster-whisper` 自动切真模型）、**回收站**、**批量操作**、**收藏**、**共享相册**、**justified 网格 + 虚拟滚动**、**智能相册**、**地点地图**、**高级筛选**、**以图搜图**

- 待补齐（M1 完成后）：关键帧、静音预览、跳秒播放
...
### v1 高级筛选（✅ 已完成，2026-08-07）

- 后端 `/api/assets` 新增 `taken_after`/`taken_before`（epoch，`>=`/`<`）、`place`（聚类网格 `"lat,lon"`，复用 smart_albums 网格判定）；与既有 media_type/favorite/tag/category/person_id 全部 AND 交叠
- 前端时间轴新增 `FilterPanel`：媒体类型/收藏/日期起止/人物/地点/标签下拉 + active 计数 + 一键清除；`queryKey` 绑定完整筛选条件
- `api.ts` `assets` 改为结构化 `AssetsFilters`；补全此前缺失的 `toggleFavorite`/批量/回收站 API 封装
- 修复前序 v1 功能长期静默的前端类型错误（esbuild 不查类型），`npx tsc --noEmit` 归零
- 新增 3 个测试；全量 117 测试通过；构建通过

### v1.1 / v2

未开始。多用户、共享链接、ASR、Live Photo、RAW、GPU 转码、移动端等见 [FEATURES.md](FEATURES.md#v11-建议) 与 [FEATURES.md](FEATURES.md#v2-远期)。

**下一项：以图搜图 ✅；剩余 v1 必备：关键帧 / 静音预览 / 跳秒播放。推荐下一项为「关键帧抽取」——以图搜图已复用 `embedding.jina_clip` 的 scene 向量管道，关键帧同样落在 scene 切分结果之上（每场景 N 张落 thumbnails 并作为 video 的预览帧），前端已具备渲染占位帧的条件。**
